...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Production-level Realtime logic: Replaces items in the existing requests array on every DB update
    const requestsSub = supabase.channel('yaninda-dynamic-sync')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'requests' }, (payload) => {
        // Only add if it's the student's own request or if we are in volunteer/admin view
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'requests' }, (payload) => {
        // Update the request in the list instantly
        setRequests(prev => prev.map(req => req.id === payload.new.id ? payload.new : req));
        
        // Update monthly limit if the student's request is marked Completed
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'volunteers' }, () => {
        // Refresh volunteers list for admin view when any volunteer data changes
        if (user?.is_admin) {
           supabase.from('volunteers').select('full_name, current_location, is_available')
