...
// --- CONFIGURATION ---
// Yeni Supabase bilgileriniz buraya eklendi
    ...
    <div className="flex flex-col h-64 bg-slate-50 rounded-2xl overflow-hidden border mt-4 animate-in fade-in zoom-in-95 duration-300">
      ...
      <div className="p-2 bg-white border-t flex gap-2">
        <input 
          ...
          onKeyPress={e => e.key === 'Enter' && sendMsg(msgInput)} 
          className="flex-1 bg-slate-50 rounded-full px-4 py-2 text-sm outline-none focus:ring-1 focus:ring-pink-200" 
          placeholder="Mesaj yaz..." 
        />
        <button onClick={() => sendMsg(msgInput)} className="bg-pink-500 text-white p-2 rounded-full active:scale-90 transition-transform flex items-center justify-center">
          <Send size={16} />
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const updateMonthlyCount = useCallback(async (uid) => {
    ...
    } catch (e) { console.error("Limit sayacı hatası", e); }
  }, []);
  ...
  const sendTelegram = async (location) => {
    try {
        // Vercel backend route (/api/telegram.js) üzerinden güvenli gönderim
      ...
      {systemMessage && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xs rounded-3xl p-6 shadow-2xl text-center border scale-in-center">
            ...
            <button onClick={() => setSystemMessage(null)} className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold shadow-lg">Tamam</button>
          </div>
        </div>
      )}
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xs rounded-[32px] p-6 shadow-2xl scale-in-center">
            <h4 className="font-black text-slate-800 text-lg mb-2 text-center">Onay</h4>
            <p className="text-sm text-slate-500 mb-6 leading-relaxed text-center"><span className="font-bold text-pink-500 underline">{selectedLocation}</span> konumuna yardım talebi gönderilsin mi?</p>
            <div className="flex flex-col gap-2">
              <button onClick={handleRequest} className="w-full bg-pink-500 text-white py-4 rounded-2xl font-black shadow-lg shadow-pink-100 active:scale-95 transition-transform">Evet, İstiyorum</button>
      ...
      <main className="max-w-md mx-auto p-5 space-y-6">
            ...
            <div className="bg-gradient-to-br from-pink-500 to-rose-400 p-8 rounded-[40px] text-white shadow-xl relative overflow-hidden">
                <div className="relative z-10 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-pink-50 text-xs mt-2 opacity-90 font-medium italic">Yanındayız. Destek mülkiyetin değil, hakkındır.</p>
                    </div>
                    <div className="text-right">
                        <span className="text-[10px] font-black bg-pink-600/50 px-2 py-1 rounded-lg uppercase">Hak</span>
        ...
        {activeTab === 'login' && (
          <div className="bg-white p-10 rounded-[50px] border border-slate-100 shadow-2xl text-center space-y-6 mt-10 animate-in fade-in slide-in-from-bottom-5">
            ...
            <button onClick={() => setActiveTab('request')} className="text-slate-400 text-[10px] font-black underline hover:text-pink-500 uppercase tracking-widest transition-colors">Öğrenci Paneli</button>
          </div>
        )}
          ...
          <section className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-white p-6 rounded-[35px] border border-slate-100 shadow-xl space-y-4">
                <div className="flex justify-between items-center px-1">
                    <div><h2 className="font-black text-slate-800 tracking-tight">Aktiflik Durumu</h2><p className="text-[10px] text-slate-400 uppercase font-black tracking-widest">{user.full_name}</p></div>
            ...
            <div className="space-y-3">
              <p className="text-[10px] font-black text-slate-400 uppercase ml-1 tracking-widest">Bekleyen Talepler</p>
                ...
                <div key={req.id} className="bg-white p-5 rounded-[32px] border-2 border-pink-50 flex justify-between items-center shadow-lg animate-in zoom-in-95 transition-all">
                  ...
                  <button onClick={() => {
                    ...
                    if (activeTasks.length > 0 && (Date.now() - new Date(req.created_at).getTime()) < 300000) {
                      setSystemMessage({ title: "Bekle", text: "Önce elindeki yardımı tamamlamalısın." }); return;
                    }
              ...
              {requests.filter(r => r.status === 'Pending').length === 0 && <p className="text-center text-slate-300 text-[11px] py-10 bg-white rounded-[32px] border-2 border-dashed border-slate-100 italic">Şu an yeni çağrı yok.</p>}
            </div>

            <div className="space-y-4 pt-2">
              <p className="text-[10px] font-black text-slate-400 uppercase ml-1 tracking-widest">Gittiğim Yerler</p>
              {requests.filter(r => (r.status === 'Claimed' || r.status === 'Delivered') && r.volunteer_name === user.full_name).map(req => (
                <div key={req.id} className={`bg-white border border-blue-100 rounded-[40px] p-6 space-y-4 shadow-sm transition-all ${req.status === 'Delivered' ? 'opacity-80 grayscale-[30%]' : ''}`}>
                  <div className="flex justify-between items-center">
                    <div><h3 className="font-black text-blue-600 text-base">{req.location}</h3><p className="text-[10px] text-slate-400 font-black uppercase tracking-tighter">{req.status === 'Delivered' ? 'Öğrenci Onayı Bekleniyor' : 'Yoldasın'}</p></div>
                    <div className="flex gap-2">
                        ...
                        {user.is_admin && req.status === 'Delivered' && (
                            <button onClick={() => handleConfirmReceipt(req.id)} className="bg-slate-900 text-white px-4 py-2.5 rounded-2xl active:scale-90 transition-all font-black text-[10px] shadow-lg flex items-center gap-2 border border-slate-700 hover:bg-slate-800"><ShieldCheck size={14} className="text-pink-400" /> Admin Onayı</button>
                        )}
                    </div>
                  </div>
        ...
        {activeTab === 'admin' && user?.is_admin && (
          <section className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-[40px] text-white shadow-2xl flex justify-between items-center">
              <div><h2 className="font-black text-xl tracking-tight">Yönetici Verileri</h2><p className="text-[10px] text-slate-400 uppercase font-black tracking-widest mt-1 opacity-70">Canlı Yanında Analizi</p></div>
            ...
            <div className="bg-white p-6 rounded-[35px] shadow-sm border border-slate-100">
                ...
                <div className="space-y-3">
                    {volunteers.filter(v => v.is_available).map(v => (
                        <div key={v.full_name} className="flex justify-between items-center p-3 bg-green-50 rounded-2xl border border-green-100 animate-in zoom-in-95">
                            <div className="flex items-center gap-3">
                                ...
                                <div><p className="text-xs font-black text-slate-700">{v.full_name}</p><p className="text-[9px] text-green-600 font-bold flex items-center gap-1"><MapPin size={10}/> {v.current_location || "Kampüste"}</p></div>
                            </div>
                    ...
                    {volunteers.filter(v => v.is_available).length === 0 && <p className="text-center text-slate-300 text-[11px] py-4 italic font-medium">Şu an aktif gönüllü görünmüyor.</p>}
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white p-5 rounded-[32px] shadow-sm border border-slate-100 text-center">
                    ...
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Başarılı Yardım</p>
                </div>
            </div>

            <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2"><TrendingUp size={14} className="text-pink-500"/> En Çok Talep Alan Yerler</h3>
      ...
      <nav className="fixed bottom-6 left-10 right-10 bg-white/80 backdrop-blur-md shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-slate-100 p-4 flex justify-around items-center z-50 rounded-[2.5rem] ring-1 ring-black/5">
        ...
        {user?.is_admin && (
          <button onClick={() => setActiveTab('admin')} className={`flex flex-col items-center gap-1 transition-all duration-300 ${activeTab === 'admin' ? 'text-pink-500 scale-110 font-black' : 'text-slate-300 font-bold'}`}>
            <BarChart3 size={24} />
            <span className="text-[9px] uppercase tracking-widest">Analiz</span>
          </button>
        )}
      </nav>
    </div>
